...
function FormGroup({ className, fields, model, horizontal, setModel = () => {}, errors = {}, setErrors = () => {}, altSetModel = false, submitAction }: Props) {
  ...
  const onChange = (
    ...
    //Run validation when user types/selects in the field
  ...
  return (
    <div className={`  ${className}`}>
      {fields.map((field, i) => {
        //Check hidden condition for the field
        if (field.hidden && field.hidden({ model, field, fields })) return null;

        //check error for the label class
        ...
        return (
          <div
            key={field.name}
            id={field.name}
            className={`relative ${field.description ? "mb-5" : "mb-5"} flex ${field?.wrapperHorizontal ? "flex-row justify-between" : "flex-col"} ${
              field.wrapperClassName
            }`}
          >
            {/* if the label is passed in the field, then show the label, else show the placeholder  */}
            {field.label ? (
